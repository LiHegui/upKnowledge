...
## Q: AI 冲击下，前端工程师的护城河是什么？

<details><summary>查看答案</summary>

**A:**

AI 对前端的冲击是**真实的**，尤其是**还原类**（按设计稿出页面）和**样板代码**（CRUD、表单、列表）部分——这类工作的稀缺性正在被快速稀释。所以如果优势还停留在「我会写 React/Vue」的语法熟练度上，确实容易被替代。

但前端的核心价值正在向**三个更上层的维度**迁移：
...
举例：同样是表单交互——
- 什么时候该用 **optimistic update**（乐观更新）？
- 什么时候必须**锁住按钮防重复提交**？
- 什么时候需要 **draft auto-save**（草稿自动保存）？

这些判断来自对**用户体验**和**边界 case** 的理解，不是 AI 凭 Prompt 生成出来的。

---

**2️⃣ 系统层面的架构决策与上下文持有能力**

> AI 没有持久记忆，上下文喂不够就会瞎给方案。

当项目复杂度上来后，下列决策都需要人来端到端 hold 住：

| 决策维度 | 典型场景 |
|---------|---------|
| 微前端拆分 | 主子应用边界、共享依赖治理 |
| 状态边界划分 | 全局 store / 局部 state / URL 状态 |
| 性能瓶颈归因 | 是渲染、是网络、还是内存泄漏？ |

真正省成本的，是有人能**端到端串起这条决策链**。

---

**3️⃣ 让 AI 在团队里可控地产出**（最重要）

> 重点不是「用不用 AI」，而是「能不能让 AI 的产出可信地进入生产」。

具体做法：

```text
✅ 维护一套 eslint / prettier / commitlint + 架构约定文件
✅ 给 Copilot / Cursor 配 rules，让它对齐团队规范
✅ Review AI PR 时重点盯：闭包依赖、竞态、内存引用
   —— 这些是 AI 容易写飘的地方
```

本质上，角色已经从「**写代码的人**」升级为「**让 AI 代码可信地进入生产的人**」。
...
> 这是一道高频串联大题，从「工具引入」层层深入，最终落到「工具调用失败兜底」的终极工程问题。以下按推进顺序完整还原。
...
## Q: 长期记忆（Long-term Memory）系统是如何设计的？如何避免记忆越来越多撑爆上下文？
...
> **长期记忆 ≠ 塞进 LLM 上下文的 Prompt。**
> 长期记忆是**外挂的数据库**（如向量库、SQL），而喂给 LLM 的是**检索出来的极少量的"当前最相关"记忆**。

长期记忆**数量无限增长，并不会直接增加 LLM 推理的 Token 成本**，真正决定 Token 消耗和思考时间的，是**检索策略和加载机制**。

### 减负四板斧

**① 核心屏障：强制 Top-K 截断（物理限流）**

无论数据库里存了多少条记录，检索器每次只允许提取最相似的 **Top-3 到 Top-10** 条记忆加载到工作记忆中。

```text
效果：相当于把一本厚字典压缩成一张"便利贴"。LLM 每次看到的上下文长度是固定的，
不会因为数据库增大而变长。
```

**② 认知简化：记忆合并压缩（Compression）**

长期记忆不存流水账，而是存**合并后的高密度信息**。

| 阶段 | 操作 | 效果 |
|------|------|------|
| 原始记录 | 20 条琐碎记忆（"说喜欢红色""又说喜欢蓝色""最后买了绿色"） | 数据量大、噪音多 |
| 压缩后 | 1 条结晶记忆（"色彩偏好多变，决策时需提供色板供选择"） | 单条信息密度高，Top-5 即可覆盖全部知识 |

**③ 时效筛选：时间权重衰减（只取"热数据"）**

查询时加权 `recent_score`，超过 3 个月的旧记忆相关性得分打 0.5 折，除非用户明确问"去年的项目"，否则默认只加载近期的活跃记忆。

**④ 分库存储：按场景隔离（Route 路由）**

不让所有记忆混在一个大池子里，按意图路由到不同库：
...
根据当前意图，**只查其中 1 个库**，而不是全部。

### 真正的瓶颈在哪？—— 检索耗时

虽然加载的 Token 数量被控制住了，但长期记忆太多可能导致**检索耗时变长**：

| 问题 | 原因 | 解决思路 |
|------|------|---------|
| 百万级记忆时 ANN 检索 | 需要几十到几百毫秒 | **多级缓存（Redis）**：高频记忆提前缓存，命中率通常超过 80% |
| 重排序（Rerank）步骤 | 可能增加 1-2 秒 | 缓存当天/本周的活跃数据，用户几乎无感知 |

### 直观对比

| 场景 | 做法 | 效果 |
|------|------|------|
| ❌ **错误认知** | 10 万条全部复制进 Prompt（约 2000 万 Token） | LLM 直接崩溃 |
| ✅ **正确工程实践** | 向量检索在海量库里捞最像的 **5 条**（约 2000 Token）喂给 LLM | Token 消耗恒定，思考时间恒定，仅多几十毫秒检索开销 |

> ⚠️ **注意**：长期记忆的数量增长，不会影响单次推理的 Token 成本和思考时长，它只会让检索系统在**海量数据中"捞针"**的难度增加，而这部分压力由向量数据库承担，而非大模型承担。设计时的关键优化点在**检索的重排序层**，这是保证"记忆虽多但每次切中要害"的关键。
