...
```js
// 递归
function deepTraverseRecursive(obj, callback) {
  callback(obj)
  for (const key in obj) {
    if (typeof obj[key] === 'object' && obj[key] !== null) {
      deepTraverseRecursive(obj[key], callback)
    }
  }
}

// 非递归（栈模拟）
function deepTraverseIterative(root, callback) {
  const stack = [root]
  while (stack.length > 0) {
