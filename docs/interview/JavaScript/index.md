...
> ⚠️ **注意**：闭包不是「内层访问外层」这么简单——核心特征是**函数被带出去执行时，仍持有定义时的作用域链**。
...
关键机制：`await expr` 会把“await 之后的剩余代码”包装成 `Promise.resolve(expr).then(剩余代码)`，**丢进微任务队列**，同时暂停当前函数。
...
async function foo() {
  console.log('4')
  await Promise.resolve()        // 暂停、将 console.log('5') 作为微任务调度
  console.log('5')
}
foo()
...
| --- | --- | --- | --- |
| 同步 | `console.log('1')` | `1` | — |
| 同步 | `setTimeout` 注册宏任务 | — | 宏：[T] |
| 同步 | `Promise.then` 注册微任务 | — | 微：[P3] |
| 同步 | `foo()` 进入，`console.log('4')` | `4` | — |
| 同步 | `await` 暂停，将 `log('5')` 丢微任务队列 | — | 微：[P3, P5] |
| 同步 | `console.log('6')` | `6` | — |
| 清空微任务 | P3 → P5 | `3` `5` | 微：[] 宏：[T] |
| 取宏任务 | T | `2` | — |

> ⚠️ **注意**：
> 1. `async` 函数本质返回一个 Promise，`await` 后面的代码等价于 `.then` 回调 → 属于**微任务**。
> 2. `fetch()` 调用本身是同步的，**`.then` 回调才是微任务**；而 XHR 的 `onload` 回调是宏任务。不要笼统地说“请求是宏任务”。
> 3. V8 7.2+ 对 `await Promise.resolve()` 做了优化，只占 1 个微任务 tick（旧引擎是 2 个）。
