...
| 阶段 | 状态 | 完成日期 |
|------|------|---------|
| 阶段一：Agent 核心认知 | ✅ 已完成 | 2026-06-18 |
| 阶段二：从零写最小 Agent | ✅ 已完成 | 2026-06-18 |
| 阶段三：记忆 + 流式 + 编排 | ⏳ 未开始 | - |
| 阶段四：综合实战 | ⏳ 未开始 | - |
...
已完成完整的 Agent 项目搭建，包含：
- 项目结构：package.json、tsconfig.json、环境配置
- tools.ts：三个工具（天气查询、网络搜索、计算器），每个工具包含 JSON Schema 描述和真实实现
- agent.ts：完整的 ReAct 循环实现，包含日志追踪、错误处理、最大步数限制
- index.ts：四个示例场景，展示不同的工具调用情况

**💻 代码练习**：

```typescript
// 已实现完整的 Agent 主循环，位于 learn-agent/src/agent.ts
// 核心逻辑：
// 1. 初始化 messages (System Prompt + User Input)
// 2. for 循环 (maxSteps = 6)
// 3. 调用 LLM，检查是否有 tool_calls
// 4. 如果有 tool_calls，执行工具，结果加入 messages，continue
// 5. 如果没有 tool_calls，返回最终答案
```

**❓ 疑问记录**：

- 如何处理工具执行失败的情况？ → 已实现 try-catch，返回结构化错误
- 如何避免死循环？ → 设置 maxSteps = 6，超出后返回友好提示

**⭐ 总结**：

Agent = LLM 大脑 + 工具手脚 + ReAct 循环。LLM 负责"决策"，代码负责"执行"。
